...
Observer_SS_Incapsuler( const SS_Interface& system )
    :Observer_Interface( TooN::Vector<>(0), TooN::Vector<>(0) ),
    system_( system.clone() )
    {}

Observer_SS_Incapsuler(const Observer_SS_Incapsuler& ss)
...
{
    return system_->state_fcn(
                                x_k_1,
                                buildFullInput(u_k,y_k)
                            );
}
...
{
    return system_->output_fcn(
                                x_k,
                                buildFullInput(u_k,TooN::Zeros( getSizeOutput() ))
                            );
}
...
{
    return system_->jacob_state_fcn(
                                x_k_1,
                                buildFullInput(u_k,y_k)
                            );
}
...
{
    return system_->jacob_output_fcn(
                                x_k,
                                buildFullInput(u_k,TooN::Zeros( getSizeOutput() ))
                            );
}
