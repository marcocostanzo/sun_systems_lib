...
/*===============CONSTRUCTORS===================*/
    ...
    TF_MIMO_DIAGONAL::TF_MIMO_DIAGONAL(const std::vector<TF_SISO_Ptr>& siso_diag_vect, double Ts):
        TF_MIMO(siso_diag_vect.size(), siso_diag_vect.size(), siso_diag_vect, Ts){}

    TF_MIMO_DIAGONAL::TF_MIMO_DIAGONAL(const std::vector<TF_SISO_Ptr>& siso_diag_vect):
        TF_MIMO_DIAGONAL(siso_diag_vect, -1){}

    TF_MIMO_DIAGONAL::TF_MIMO_DIAGONAL(int dim_diag, const TF_SISO& base_tf_system, double Ts):
        TF_MIMO(dim_diag, dim_diag, Ts)
        {
            ...
            for(int i = 0; i < dim_diag; i++)
                _siso_vect.push_back( TF_SISO_Ptr( base_tf_system.clone() ) );

        }

    TF_MIMO_DIAGONAL::TF_MIMO_DIAGONAL(int dim_diag, const TF_SISO& base_tf_system):
    ...
    std::vector<TF_SISO_Ptr> TF_MIMO_DIAGONAL::getSISOMatrixVect(){
        return expandDiagToMatrix(_siso_vect);
    }

    std::vector<TF_SISO_Ptr> TF_MIMO_DIAGONAL::getSISODiagVect(){
        std::vector<TF_SISO_Ptr> out;
    ...
    void TF_MIMO_DIAGONAL::setSISOMatrixVect(const std::vector<TF_SISO_Ptr>& siso_matrix_vect ){
        ...
        vector<TF_SISO_Ptr> diag_vec;   

        for(int i = 0; i<_mimo_dim_input; i++)
            diag_vec.push_back( TF_SISO_Ptr( siso_matrix_vect[i*_mimo_dim_input + i]->clone() ) );
    ...
    void TF_MIMO_DIAGONAL::setSISODiagVect(const std::vector<TF_SISO_Ptr>& siso_diag_vect ){
        ...
        _siso_vect.clear();
        for( const auto &element : siso_diag_vect ){
    ...
    vector<TF_SISO_Ptr> TF_MIMO_DIAGONAL::expandDiagToMatrix(const vector<TF_SISO_Ptr>& siso_diag_vect){

        vector<TF_SISO_Ptr> siso_matrix_vect;

        for( int i = 0; i<siso_diag_vect.size(); i++){
            siso_matrix_vect.push_back( TF_SISO_Ptr( siso_diag_vect[i]->clone() ) );
            if( !( i == (siso_diag_vect.size()-1) ) ){
                for( int j = 0; j<siso_diag_vect.size(); j++ ){
                    siso_matrix_vect.push_back( TF_SISO_Ptr( new TF_SISO() ) );
                }
            }
        }
    ...
    TF_SISO* TF_MIMO_DIAGONAL::operator()(int diag_index){
        return _siso_vect[diag_index].get();
    }
    /*==============================================*/